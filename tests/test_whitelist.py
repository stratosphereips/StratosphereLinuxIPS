...
@pytest.mark.parametrize(
    "mock_ip_info, mock_org_info, ip, org, expected_result", [
        ({'asn': {'asnorg': 'microsoft'}}, [json.dumps(['microsoft']), json.dumps([])], "91.121.83.118", "microsoft",
         True),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('www.google.com', 'google', json.dumps([]), None),  # no org domain info
    ]
)
...
@pytest.mark.parametrize('evidence_data, expected_result', [
    ({'attacker': MagicMock(attacker_type='IP', value='1.2.3.4', direction=Direction.SRC)}, True),
    # Whitelisted source IP
    ({'victim': MagicMock(victim_type='DOMAIN', value='example.com', direction=Direction.DST)}, True),
...
@pytest.mark.parametrize('profile_ip, mac_address, direction, expected_result, whitelisted_macs', [
    ('1.2.3.4', 'b1:b1:b1:c1:c2:c3', Direction.SRC, None,
     {'b1:b1:b1:c1:c2:c3': {'from': 'src', 'what_to_ignore': 'alerts'}}),
    ('5.6.7.8', 'a1:a2:a3:a4:a5:a6', Direction.DST, True,
...
@pytest.mark.parametrize(
    ...
    [
        ({'attacker_type': IoCType.IP.name, 'value': '1.2.3.4', 'direction': Direction.SRC}, False),
        ({'victim_type': IoCType.DOMAIN.name, 'value': 'example.com', 'direction': Direction.DST}, True),
        ({'attacker_type': IoCType.IP.name, 'value': '8.8.8.8', 'direction': Direction.SRC}, False),
    ])
def test_is_part_of_a_whitelisted_org(ioc_data, expected_result, mock_db):
    whitelist = ModuleFactory().create_whitelist_obj(mock_db)
    mock_db.get_all_whitelist.return_value = {
        'organizations': json.dumps({'google': {'from': 'src', 'what_to_ignore': 'both'}})
    }
...
@pytest.mark.parametrize('attacker_data, expected_result', [
    (MagicMock(attacker_type=IoCType.IP.name, value='1.2.3.4', direction=Direction.SRC), False),
    (MagicMock(attacker_type=IoCType.DOMAIN.name, value='example.com', direction=Direction.DST), False),
])
...
@pytest.mark.parametrize('victim_data, expected_result', [
    (MagicMock(victim_type=IoCType.IP.name, value='1.2.3.4', direction=Direction.SRC), None),
    (MagicMock(victim_type=IoCType.DOMAIN.name, value='example.com', direction=Direction.DST), None),
])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('1.1.1.1', 'Cloudflare', json.dumps(['AS13335']), {'asn': {'number': 'AS15169', 'asnorg': 'Google'}}, False),
        ('9.9.9.9', 'IBM', json.dumps(['AS36459']), {}, None),
        ('9.9.9.9', 'IBM', json.dumps(['AS36459']), {'asn': {'number': 'Unknown'}}, False),
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
                ...
                MagicMock(saddr="9.8.7.6", daddr="1.2.3.4", smac="b1:b1:b1:c1:c2:c3", dmac="a1:a2:a3:a4:a5:a6",
                          type_="http", server_name="example.org"),
...
@pytest.mark.parametrize(
    'whitelist_data, expected_ips, expected_domains, expected_orgs, expected_macs', [
