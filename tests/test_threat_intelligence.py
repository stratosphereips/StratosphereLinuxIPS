...
def test_parse_ti_file(database, output_queue):
    threatintel = create_threatintel_instance(output_queue, database)
...
def test_check_local_ti_files_for_update(
        output_queue, current_hash, old_hash, expected_return, mocker, database
):
    """
