...
def prepare_configuration(cleanup=False):
    ...
    if not cleanup:
        ...
        # because the creator is run in port 9002 and the first iris is
        # using 9010
        # linux keeps port binidn gfor 4o secondsadfter the process is killed
        original_conn_string = re.sub(
            r"/udp/9002/", "/udp/9010/", original_conn_string
        )
...
def test_messaging(
    ...
    iris_log_file = os.path.join(output_dir_create, "iris_log.txt")

    print("running peercli ...")
    # Define the executable and config file paths for iris key creator
    peercli_executable_path = os.path.abspath("./modules/irisModule/peercli")
    iris_creator_config = os.path.abspath(
        "tests/integration_tests/config/iris_creator_config.yaml"
    )

    cmd_to_start_a_peer_in_cli = [
        str(peercli_executable_path),
        "--conf",
        str(iris_creator_config),
    ]

    with open(iris_log_file, "w") as log_file:
        redis_process = subprocess.Popen(["redis-server", "--port", "6633"])

        countdown(5, " starting iris creator")
        key_creator = subprocess.Popen(
            cmd_to_start_a_peer_in_cli, stdout=log_file, stderr=log_file
        )

        countdown(15, "sigterm")
        # send a SIGTERM to both processes
        redis_process.terminate()
        os.kill(key_creator.pid, 15)

        print("SIGTERM sent. killing iris' creator")
        redis_process.kill()
        os.kill(key_creator.pid, 9)
    ...
    try:
        with open(output_file, "w") as log_file:
            with open(output_file_peer, "w") as iris_log_file:
                ...
                # todo we can rename peercli to iris
