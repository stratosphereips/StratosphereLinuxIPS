...
def test_check_if_enough_dstips_to_trigger_an_evidence_no_cache(mock_db):
    """
    Test the check_if_enough_dstips_to_trigger_an_evidence method when there is no cached threshold.
    """
    horizontal_ps = ModuleFactory().create_horizontal_portscan_obj(mock_db)
    profileid = 'profile_1.1.1.1'
    timewindow = 'timewindow0'
    ...
    enough = horizontal_ps.check_if_enough_dstips_to_trigger_an_evidence(key, cur_amount_of_dstips)
    assert enough is True
    
def test_check_if_enough_dstips_to_trigger_an_evidence_less_than_minimum(mock_db):
    horizontal_ps = ModuleFactory().create_horizontal_portscan_obj(mock_db)
    profileid = 'profile_1.1.1.1'
    timewindow = 'timewindow0'
    dport = 5555
...
def test_check_if_enough_dstips_to_trigger_an_evidence_equal_min_dips(mock_db):
    ...
    enough = horizontal_ps.check_if_enough_dstips_to_trigger_an_evidence(key, amount_of_dips)
    assert enough is True
...
def test_get_not_estab_dst_ports(mock_db):
    ...
    mock_dports = {
        ...
        443: {'dstips': {'8.8.8.8': {'dstports': {443: 20}}, '1.1.1.1': {'dstports': {443: 30}}}}
    }
    mock_db.get_data_from_profile_tw.return_value = mock_dports

    dports = horizontal_ps.get_not_estab_dst_ports(protocol, state, profileid, twid)
...
def test_get_cache_key_empty_dport():
    ...
    cache_key = horizontal_ps.get_cache_key(profileid, twid, dport)
    expected_key = f'{profileid}:{twid}:dport:None:HorizontalPortscan'
    assert cache_key == expected_key  
...
@patch('modules.network_discovery.horizontal_portscan.HorizontalPortscan.get_not_estab_dst_ports')
def test_check_broadcast_or_multicast_address(mock_get_not_estab_dst_ports, mock_db):
...
def test_decide_if_time_to_set_evidence_or_combine_empty_alerted(mock_db):
    ...
    cache_key = horizontal_ps.get_cache_key(evidence['profileid'], evidence['twid'], evidence['dport'])
    result = horizontal_ps.decide_if_time_to_set_evidence_or_combine(evidence, cache_key)
...
def test_combine_evidence_different_keys(mock_db):
    ...
    horizontal_ps.pending_horizontal_ps_evidence = {
        'profile_1.1.1.1-timewindow0-Not Established-TCP-80': [(1, 10, ['uid1'], 5)],
        'profile_2.2.2.2-timewindow1-Not Established-UDP-53': [(2, 20, ['uid2', 'uid3'], 10)],
        'profile_3.3.3.3-timewindow2-Not Established-TCP-443': [(3, 30, ['uid4'], 15), (4, 40, ['uid5'], 20), (5, 50, ['uid6'], 25)]
    }
    mock_db.get_port_info.return_value = "HTTP"
    mock_db.set_evidence.return_value = None
...
def test_combine_evidence_multiple_keys(mock_db):
    horizontal_ps = ModuleFactory().create_horizontal_portscan_obj(mock_db)
    horizontal_ps.pending_horizontal_ps_evidence = {
        'profile_1.1.1.1-timewindow0-Not Established-TCP-80': [(1, 10, ['uid1'], 5), (2, 20, ['uid2'], 10), (3, 30, ['uid3'], 15)],
        'profile_2.2.2.2-timewindow1-Not Established-UDP-53': [(4, 40, ['uid4'], 20), (5, 50, ['uid5'], 25), (6, 60, ['uid6'], 30)]
    }
...
def test_check_multicast_address(mock_db):
    ...
    with patch.object(horizontal_ps, 'get_not_estab_dst_ports') as mock_get_not_estab_dst_ports:
        horizontal_ps.check(profileid, twid)
        mock_get_not_estab_dst_ports.assert_not_called() 
