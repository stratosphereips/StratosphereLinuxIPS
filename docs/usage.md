...
- **alerts-summary.log in `output/alerts/`** - when the `alert_summary` module is enabled, stores one analyst-facing paragraph per alert based on the correlated evidence and the shared LLM reply.
- **`llm-summary/alert_summary.log` in the output folder** - when the `alert_summary` module is enabled, stores operational logs for alert queueing, LLM requests, responses, failures, and other module activity according to `alert_summary.log_verbosity`.
