...
Evidence also includes records that did not cross the alert threshold. The default table shows individual evidence, bounded to 100 rows per page. **Group by host and type** is optional and shows evidence, triggering-flow, and alert-link counts. Selecting an aggregate shows its newest individual evidence, and selecting an individual record shows the conn and alternative flows whose UIDs caused it. Alert to evidence to flow drill-down remains available after Redis time windows expire.
