...
the sqlite database can be exported to json or csv format. 
...
for now, the ```export_format``` parameter supports csv or json formats only.

the exported flows are stored in a file called ```labeled_flows.json``` or ```labeled_flows.csv``` in the output directory.
