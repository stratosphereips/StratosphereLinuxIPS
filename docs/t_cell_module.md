...
The trace path is always resolved under the t_cell module output directory
...
under `<run_output_dir>/t_cell/` before writing the file.
...
primary source, then enriches the page with `t_cell/t_cell.log` and
`t_cell/t_cell_trace.jsonl` when those files exist. This means:
...
- `create_log_file`: create `<run_output_dir>/t_cell/t_cell.log`
...
- `decision_trace_file`: JSONL audit file for threshold explanations, always
  created under the t_cell module output directory
