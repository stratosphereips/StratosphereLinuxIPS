...
class InputProcess(multiprocessing.Process):
    ...
    def read_zeek_files(self) -> int:
        ...
        lines = 0
        while True:
            # reoder zeek_files to make sure slips reads conn.log first
            zeek_files = list(zeek_files)
            for index, file in enumerate(zeek_files):
                if 'conn' in file:
                    # move conn.log to be the first element
                    zeek_files[0], zeek_files[index] = zeek_files[index] , zeek_files[0]
                    break
