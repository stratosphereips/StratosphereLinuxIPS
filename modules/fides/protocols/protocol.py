...
class Protocol:
    ...
    def _evaluate_interaction(
        self, peer: PeerTrustData, satisfaction: Satisfaction, weight: Weight
    ) -> PeerTrustData:
        """Callback to evaluate and save new trust data for given peer."""
        return self._evaluate_interactions(
            {peer.peer_id: (peer, satisfaction, weight)}
        )[peer.peer_id]

    def _evaluate_interactions(
        self, data: Dict[PeerId, Tuple[PeerTrustData, Satisfaction, Weight]]
    ) -> TrustMatrix:
        ...
        for _, (peer_trust, satisfaction, weight) in data.items():
            updated_trust = process_service_interaction(
                self._configuration, peer_trust, satisfaction, weight
            )
        ...
        self._bridge.send_peers_reliability(
            {p.peer_id: p.service_trust for p in trust_matrix.values()}
        )
        return trust_matrix
