...
class OpinionAggregator:
    ...
    def evaluate_alert(
        self, peer_trust: PeerTrustData, alert: Alert
    ) -> SlipsThreatIntelligence:
        """Evaluates given data about alert and produces aggregated intelligence for Slips."""

        alert_trust = max(
            self.__configuration.alert_trust_from_unknown,
            peer_trust.service_trust,
        )
        score = alert.score
        confidence = alert.confidence * alert_trust
        return SlipsThreatIntelligence(
            score=score, confidence=confidence, target=alert.target
        )

    def evaluate_intelligence_response(
        self,
        target: Target,
        ...
        reports = [
            PeerReport(
                report_ti=ti.intelligence, reporter_trust=trust_matrix[peer_id]
            )
            for peer_id, ti in data.items()
        ]
        ti = self.__ti_aggregation.assemble_peer_opinion(data=reports)
        return SlipsThreatIntelligence(
            score=ti.score, confidence=ti.confidence, target=target
        )
