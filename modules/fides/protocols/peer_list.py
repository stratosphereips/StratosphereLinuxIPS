...
class PeerListUpdateProtocol:
    ...
    def handle_peer_list_updated(self, peers: List[PeerInfo]):
        ...
        trust_data = self.__trust_db.get_peers_trust_data(
            [p.id for p in peers]
        )
        known_peers = {
            peer_id
            for peer_id, trust in trust_data.items()
            ...
            for peer in [p for p in peers if p.id not in known_peers]:
                ...
                peer_trust = (
                    self.__trust_protocol.determine_and_store_initial_trust(
                        peer, get_recommendations=False
                    )
                )
                new_trusts.append(peer_trust)
                # get recommendations for this peer
                self.__recommendation_protocol.get_recommendation_for(
                    peer, connected_peers=list(known_peers)
                )
            # send only updated trusts to the network layer
            self.__bridge.send_peers_reliability(
                {p.peer_id: p.service_trust for p in new_trusts}
            )
