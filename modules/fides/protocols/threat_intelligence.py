...
class ThreatIntelligenceProtocol(Protocol):
    ...
    def handle_intelligence_request(
        ...
        if not peer_trust:
            ...
            peer_trust = (
                self.__trust_protocol.determine_and_store_initial_trust(sender)
            )
        ...
        self._evaluate_interaction(
            peer_trust, SatisfactionLevels.Ok, Weight.INTELLIGENCE_REQUEST
        )

    def handle_intelligence_response(
    ...
    def __filter_ti(
        self, ti: Optional[SlipsThreatIntelligence], peer_trust: PeerTrustData
    ) -> Optional[SlipsThreatIntelligence]:
        if ti is None:
            return None

        peers_allowed_levels = [
            p.confidentiality_level
            for p in self._configuration.trusted_organisations
            if p.id in peer_trust.organisations
        ]
        ...
        ti.confidentiality = (
            ti.confidentiality
            if ti.confidentiality
            else self._configuration.data_default_level
        )
