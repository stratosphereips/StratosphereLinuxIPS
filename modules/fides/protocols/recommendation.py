...
class RecommendationProtocol(Protocol):
    ...
    def get_recommendation_for(
        self, peer: PeerInfo, connected_peers: Optional[List[PeerId]] = None
    ):
        ...
        if not self.__rec_conf.enabled:
            logger.debug(
                f"Recommendation protocol is disabled. NOT getting recommendations for Peer {peer.id}."
            )
            return
        ...
        connected_peers = (
            ...
            else self.__trust_db.get_connected_peers()
        )
        recipients = self.__get_recommendation_request_recipients(
            peer, connected_peers
        )
        if recipients:
            self.__bridge.send_recommendation_request(
                recipients=recipients, peer=peer.id
            )
        else:
            logger.debug(
                "No peers are trusted enough to ask them for recommendation!"
            )

    def handle_recommendation_request(
        self, request_id: str, sender: PeerInfo, subject: PeerId
    ):
        ...
        self.__bridge.send_recommendation_response(
            request_id, sender.id, subject, recommendation
        )
    ...
    def handle_recommendation_response(
        self, responses: List[PeerRecommendationResponse]
    ):
        ...
        assert all(
            responses[0].subject == r.subject for r in responses
        ), "Responses are not for the same subject!"

        subject = self.__trust_db.get_peer_trust_data(responses[0].subject)
        if subject is None:
            logger.warn(
                f"Received recommendation for subject {responses[0].subject} that does not exist!"
            )
            return

        recommendations = {r.sender.id: r.recommendation for r in responses}
        trust_matrix = self.__trust_db.get_peers_trust_data(
            list(recommendations.keys())
        )

        # check that the data are consistent
        assert (
            len(trust_matrix) == len(responses) == len(recommendations)
        ...
        # and dispatch event
        self.__bridge.send_peers_reliability(
            {p.peer_id: p.service_trust for p in updated_matrix.values()}
        )
        ...
        interaction_matrix = {
            p.peer_id: (
                p,
                SatisfactionLevels.Ok,
                Weight.RECOMMENDATION_RESPONSE,
            )
    ...
    def __is_zero_recommendation(recommendation: Recommendation) -> bool:
        return (
            recommendation.competence_belief == 0
            and recommendation.integrity_belief == 0
            and recommendation.service_history_size == 0
            and recommendation.recommendation == 0
            and recommendation.initial_reputation_provided_by_count == 0
        )

    def __get_recommendation_request_recipients(
        self, subject: PeerInfo, connected_peers: List[PeerInfo]
    ) -> List[PeerId]:
        recommenders: List[PeerInfo] = []
        require_trusted_peer_count = (
            self.__rec_conf.required_trusted_peers_count
        )
        ...
        if self.__rec_conf.only_preconfigured:
            preconfigured_peers = set(
                p.id for p in self._configuration.trusted_peers
            )
            preconfigured_organisations = set(
                p.id for p in self._configuration.trusted_organisations
            )
            ...
            if len(recommenders) > 0:
                ...
                recommenders = [
                    p
                    for p in recommenders
                    if p.id in preconfigured_peers
                    or preconfigured_organisations.intersection(
                        p.organisations
                    )
                ]
            else:
                ...
                recommenders = self.__trust_db.get_peers_info(
                    list(preconfigured_peers)
                ) + self.__trust_db.get_peers_with_organisations(
                    list(preconfigured_organisations)
                )
            ...
            recommenders = (
                self.__trust_db.get_peers_with_geq_recommendation_trust(
                    trusted_peer_threshold
                )
            )
            ...
            if len(recommenders) <= self.__rec_conf.peers_max_count:
                ...
                recommenders += (
                    self.__trust_db.get_peers_with_geq_service_trust(
                        trusted_peer_threshold
                    )
                )
        ...
        candidates = list(
            self.__trust_db.get_peers_trust_data(recommenders).values()
        )
        candidates = [c for c in candidates if c.peer_id != subject.id]
        # check if we can proceed
        if (
            len(candidates) == 0
            or len(candidates) < require_trusted_peer_count
        ):
            logger.debug(
        ...
        return [p.peer_id for p in candidates][
            : self.__rec_conf.peers_max_count
        ]
