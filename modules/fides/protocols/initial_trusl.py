...
class InitialTrustProtocol:
    ...
    def determine_and_store_initial_trust(
        self, peer: PeerInfo, get_recommendations: bool = False
    ) -> PeerTrustData:
        ...
        if existing_trust is not None:
            logger.debug(
                f"There's an existing trust for peer {peer.id}: ST: {existing_trust.service_trust}"
            )
        ...
        pre_trusted_peer = [
            p
            for p in self.__configuration.trusted_peers
            if trust.peer_id == p.id
        ]
        ...
        peers_orgs = [
            org
            for org in self.__configuration.trusted_organisations
            if org.id in peer.organisations
        ]
        if peers_orgs:
            logger.debug(
                f"Peer {peer.id} has known organisations.", peers_orgs
            )
            ...
            logger.debug(
                "Main organisation selected, computing trust",
        ...
        trust = process_service_interaction(
    ...
    @staticmethod
    def __inherit_trust(
        trust: PeerTrustData, parent: TrustedEntity
    ) -> PeerTrustData:
        ...
        if parent.enforce_trust:
            ...
            logger.debug(
                f"Enforced trust, leaving service trust to: {trust.service_trust}."
            )
