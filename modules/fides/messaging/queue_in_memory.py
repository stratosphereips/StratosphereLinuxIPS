...
class InMemoryQueue(Queue):
    ...
    def __init__(self, on_message: Optional[Callable[[str], None]] = None):
        ...
        self.__on_message: Callable[[str], None] = (
            on_message if on_message else default_on_message
        )

    def send(
        self, serialized_data: str, should_wait_for_join: bool = False, **argv
    ):
        ...
        th = threading.Thread(
            target=lambda: self.__on_message(serialized_data)
        )
        th.start()
    ...
    def __exception(data: str):
        raise Exception(
            f"No on_message set! Call listen before calling send! Data: {data}"
        )
