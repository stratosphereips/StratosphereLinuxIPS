...
class MessageHandler:
    ...
    def __init__(
        ...
        on_recommendation_response: Callable[
            [List[PeerRecommendationResponse]], None
        ],
        on_alert: Callable[[PeerInfo, Alert], None],
        on_intelligence_request: Callable[[str, PeerInfo, Target], None],
        on_intelligence_response: Callable[
            [List[PeerIntelligenceResponse]], None
        ],
        on_unknown: Optional[Callable[[NetworkMessage], None]] = None,
        on_error: Optional[
            Callable[[Union[str, NetworkMessage], Exception], None]
        ] = None,
    ):
        # self.logger = None
    ...
    def on_message(self, message: NetworkMessage):
        ...
        if message.version != self.version:
            logger.warn(
                f"Unknown message version! This handler supports {self.version}.",
                message,
            )
        ...
        func = execution_map.get(
            message.type, lambda data: self.__on_unknown_message(message)
        )
        ...
        try:
            ...
            logger.error(
                f"Error when executing handler for message: {message.type}.",
                ex,
            )
            if self.__on_error:
    ...
    def on_error(
        self, original_data: str, exception: Optional[Exception] = None
    ):
        """
        ...
        if self.__on_error:
            self.__on_error(
                original_data,
                exception if exception else Exception("Unknown data type!"),
            )

    def __on_unknown_message(self, message: NetworkMessage):
        logger.warn("Unknown message handler executed!")
    ...
    def __on_nl2tl_peer_list(self, data: Dict):
        ...
        peers = [
            from_dict(data_class=PeerInfo, data=peer) for peer in data["peers"]
        ]
    ...
    def __on_recommendation_request(
        self, request_id: str, sender: PeerInfo, subject: PeerId
    ):
        return self.__on_recommendation_request_callback(
            request_id, sender, subject
        )
        ...
        responses = [
            PeerRecommendationResponse(
                ...
                recommendation=from_dict(
                    data_class=Recommendation,
                    data=single["payload"]["recommendation"],
                ),
            )
            for single in data
        ]
    ...
    def __on_recommendation_response(
        self, recommendations: List[PeerRecommendationResponse]
    ):
    ...
    def __on_intelligence_request(
        self, request_id: str, sender: PeerInfo, target: Target
    ):
        return self.__on_intelligence_request_callback(
            request_id, sender, target
        )
    ...
    def __on_nl2tl_intelligence_response(self, data: Dict):
        ...
        try:
            responses = [
                PeerIntelligenceResponse(
                    sender=from_dict(
                        data_class=PeerInfo, data=single["sender"]
                    ),
                    intelligence=from_dict(
                        data_class=ThreatIntelligence,
                        data=single["payload"]["intelligence"],
                    ),
            ...
            print(
                "Error in Fides message_handler.py __on_nl2tl_intelligence_response(): ",
                e.__str__(),
            )
    ...
    def __on_intelligence_response(
