...
from ..evaluation.service.interaction import (
    Satisfaction,
    Weight,
    SatisfactionLevels,
)
...
class TIEvaluation:
    def evaluate(
        ...
        **kwargs,
        ...
        raise NotImplementedError("Use implementation rather then interface!")
...
class EvenTIEvaluation(TIEvaluation):
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        return {
            p.peer_id: (p, self.__satisfaction, self._weight())
...
class DistanceBasedTIEvaluation(TIEvaluation):
    ...
    def _build_evaluation(
        ...
        return {
            p.peer_id: (p, satisfactions[p.peer_id], self._weight())
            for p in trust_matrix.values()
        }
    ...
    def _satisfaction(
        ...
        return (
            1 - (abs(baseline_score - report_score) / 2) * report_confidence
        ) * baseline_confidence
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        if not ti:
            ti = aggregated_ti
            logger.warn(
                f"No local threat intelligence available for target {ti.target}! "
...
class WeighedDistanceToLocalTIEvaluation(TIEvaluation):
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        distance_data = self.__distance.evaluate(
            aggregated_ti, responses, trust_matrix, **kwargs
        )
        local_data = self.__local.evaluate(
            aggregated_ti, responses, trust_matrix, **kwargs
        )
...
class MaxConfidenceTIEvaluation(TIEvaluation):
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        local_weight = (
            min(1 - distance_weight, local_ti.confidence) if local_ti else 0
        )
        local_data = (
            self.__local.evaluate(
                aggregated_ti, responses, trust_matrix, **kwargs
            )
            if local_weight > 0
            else zero_dict
        ...
        def aggregate(peer: PeerId):
            return (
                distance_weight * distance_data[peer][1]
                + local_weight * local_data[peer][1]
                + even_weight * even_data[peer][1]
            )

        return {
            p.peer_id: (p, aggregate(p.peer_id), self._weight())
            for p in trust_matrix.values()
        }
...
class ThresholdTIEvaluation(TIEvaluation):
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        return (
            self.__higher.evaluate(aggregated_ti, responses, trust_matrix)
