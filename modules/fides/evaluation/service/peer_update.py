...
def update_service_data_for_peer(
    ...
    service_trust_own_experience = history_factor * (
...
def __compute_fading_factor(
    configuration: TrustModelConfiguration, service_history: ServiceHistory
) -> List[float]:
    """
...
def __compute_competence_belief(
    service_history: ServiceHistory, fading_factor: List[float]
) -> float:
    """
    ...
    """
    assert len(service_history) == len(
        fading_factor
    ), "Service history must have same length as fading factors."

    normalisation = sum(
        [
            service.weight * fading
            for service, fading in zip(service_history, fading_factor)
        ]
    )
    belief = sum(
        [
...
def __compute_integrity_belief(
    ...
    history_size = len(service_history)
    weight_mean = (
        sum([service.weight for service in service_history]) / history_size
    )
    ...
    sat = sum(
        [
            (
                service.satisfaction * weight_mean * fading_mean
                - competence_belief
            )
            ** 2
