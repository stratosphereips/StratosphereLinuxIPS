...
class TIAggregation:
    ...
    def assemble_peer_opinion(
        self, data: List[PeerReport]
    ) -> ThreatIntelligence:
        """
...
class AverageConfidenceTIAggregation(TIAggregation):
    ...
    def assemble_peer_opinion(
        ...
        weighted_reporters = (
            [trust / normalize_net_trust_sum for trust in reporters_trust]
            if normalize_net_trust_sum > 0
            else [0] * len(reporters_trust)
        )

        combined_score = sum(
            r.score * w for r, w, in zip(reports_ti, weighted_reporters)
        )
        combined_confidence = sum(
            r.confidence * w for r, w, in zip(reports_ti, reporters_trust)
        ) / len(reporters_trust)

        return ThreatIntelligence(
            score=combined_score, confidence=combined_confidence
        )
...
class WeightedAverageConfidenceTIAggregation(TIAggregation):
    ...
    def assemble_peer_opinion(
        ...
        combined_confidence = sum(
            r.confidence * w for r, w, in zip(reports_ti, weighted_reporters)
        )
...
class StdevFromScoreTIAggregation(TIAggregation):
    ...
    def assemble_peer_opinion(
        ...
        merged_score = [
            r.score * r.confidence * w
            for r, w, in zip(reports_ti, weighted_reporters)
        ]
