...
def create_recommendation_history_for_peer(
    ...
    :return:
    """
    rs_ik = __compute_recommendation_satisfaction_parameter(
        recommendation, er_ij, ecb_ij, eib_ij
    )
    rw_ik = __compute_weight_of_recommendation(
        configuration, recommendation, history_factor
    )

    updated_history = peer.recommendation_history + [
        RecommendationHistoryRecord(
            satisfaction=rs_ik, weight=rw_ik, timestamp=now()
        )
    ]
    ...
    if len(updated_history) > configuration.recommendations.history_max_size:
        ...
        updated_history = updated_history[
            last_idx
            - configuration.recommendations.history_max_size : last_idx
        ]
...
def __compute_recommendation_satisfaction_parameter(
    recommendation: Recommendation, er_ij: float, ecb_ij: float, eib_ij: float
) -> float:
    """
    ...
    """
    r_diff = (
        (1 - abs(recommendation.recommendation - er_ij) / er_ij)
        if er_ij > 0
        else 0
    )
    cb_diff = (
        (1 - abs(recommendation.competence_belief - ecb_ij) / ecb_ij)
        if ecb_ij > 0
        else 0
    )
    ib_diff = (
        (1 - abs(recommendation.integrity_belief - eib_ij) / eib_ij)
        if eib_ij > 0
        else 0
    )
...
def __compute_weight_of_recommendation(
    ...
    used_peers = (
        recommendation.initial_reputation_provided_by_count
        / configuration.recommendations.peers_max_count
    )
