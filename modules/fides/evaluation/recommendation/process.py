...
from ...evaluation.recommendation.new_history import (
    create_recommendation_history_for_peer,
)
from ...evaluation.recommendation.peer_update import (
    update_recommendation_data_for_peer,
)
...
def process_new_recommendations(
    ...
    for peer in recommendations.keys():
        assert (
            matrix[peer] is not None
        ), f"Peer {peer} is not present in peer matrix."

    er_ij = __estimate_recommendation(matrix, recommendations)
    ecb_ij, eib_ij = __estimate_competence_integrity_belief(
    ...
    updated_subject_trust = dataclasses.replace(
        subject,
    ...
    peers_updated_matrix = {
        updated_subject_trust.peer_id: updated_subject_trust
    }
    ...
    for peer_id, recommendation in recommendations.items():
        ...
        updated_peer = update_recommendation_data_for_peer(
            configuration=configuration, peer=peer, new_history=new_history
        )
...
def __estimate_recommendation(
    matrix: TrustMatrix, recommendations: Dict[PeerId, Recommendation]
) -> float:
    """
    ...
    normalisation = sum(
        [
            matrix[peer].recommendation_trust
            * response.initial_reputation_provided_by_count
...
def __estimate_competence_integrity_belief(
    ...
    for peer, response in recommendations.items():
        trust_history_size = (
            matrix[peer].recommendation_trust * response.service_history_size
        )
        # rt_ik * sh_kj
