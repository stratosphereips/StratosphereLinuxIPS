...
def update_recommendation_data_for_peer(
    ...
    integrity_belief = __compute_integrity_belief(
        new_history, fading_factor, competence_belief
    )
    integrity_discount = compute_discount_factor()

    history_factor = (
        len(new_history) / configuration.recommendations.history_max_size
    )

    # (rh_ik / rh_max) * (rcb_ik -0.5 * rib_ik) -> where -0.5 is discount factor
    reputation_trust_own_experience = history_factor * (
        competence_belief + integrity_discount * integrity_belief
    )
    # (1 - (rh_ik / rh_max)) * r_ik
    reputation_experience = (1 - history_factor) * peer.reputation
    # and now add both parts together
    recommendation_trust = (
        reputation_trust_own_experience + reputation_experience
    )

    updated_trust = dataclasses.replace(
        peer,
...
def __compute_competence_belief(
    recommendation_history: RecommendationHistory, fading_factor: List[float]
) -> float:
    """
    ...
    """
    assert len(recommendation_history) == len(
        fading_factor
    ), "Recommendation history must have same length as fading factors."

    normalisation = sum(
        [
            recommendation.weight * fading
            for recommendation, fading in zip(
                recommendation_history, fading_factor
            )
        ]
    )
    ...
    belief = sum(
        [
            service.satisfaction * service.weight * fading
            for service, fading in zip(recommendation_history, fading_factor)
        ]
    )
...
def __compute_integrity_belief(
    recommendation_history: RecommendationHistory,
    fading_factor: List[float],
    recommendation_competence_belief: float,
) -> float:
    """
    ...
    history_size = len(recommendation_history)
    weight_mean = (
        sum(service.weight for service in recommendation_history)
        / history_size
    )
    ...
    sat = sum(
        (
            recommendation.satisfaction * weight_mean * fading_mean
            - recommendation_competence_belief
        )
        ** 2
