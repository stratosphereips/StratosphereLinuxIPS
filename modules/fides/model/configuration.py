...
def load_configuration(file_path: str) -> TrustModelConfiguration:
    with open(file_path, "r") as stream:
        try:
            ...
            Logger("config_loader").error(
                f"It was not possible to load file! {exc}."
            )
            raise exc
...
def __parse_config(data: dict) -> TrustModelConfiguration:
    return TrustModelConfiguration(
        privacy_levels=[
            PrivacyLevel(name=level["name"], value=level["value"])
        ...
        recommendations=RecommendationsConfiguration(
            ...
            only_connected=data["trust"]["recommendations"][
                "useOnlyConnected"
            ],
            only_preconfigured=data["trust"]["recommendations"][
                "useOnlyPreconfigured"
            ],
            required_trusted_peers_count=data["trust"]["recommendations"][
                "requiredTrustedPeersCount"
            ],
            trusted_peer_threshold=data["trust"]["recommendations"][
                "trustedPeerThreshold"
            ],
            peers_max_count=data["trust"]["recommendations"]["peersMaxCount"],
            history_max_size=data["trust"]["recommendations"][
                "historyMaxSize"
            ],
        ),
        ...
        network_opinion_cache_valid_seconds=data["trust"][
            "networkOpinionCacheValidSeconds"
        ],
        interaction_evaluation_strategy=__parse_evaluation_strategy(data),
        ti_aggregation_strategy=TIAggregationStrategy[
            data["trust"]["tiAggregationStrategy"]
        ](),
        database=(
            data["database"] if "database" in data else "fides_p2p_db.sqlite"
        ),
    )
