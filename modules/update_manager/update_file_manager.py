...
class UpdateFileManager:
    def __init__(self, outputqueue, rdb):
        ...
        self.slips_logfile = self.rdb.get_stdfile("stdout")
    ...
    def get_feed_details(self, feeds_path):
        ...
        for line in feeds.splitlines():
            ...
            if url.startswith(';'):
                feed = url.split('/')[-1]
                if self.rdb.get_TI_file_info(feed):
                    self.rdb.delete_feed(feed)
                    # to avoid calling delete_feed again with the same feed
                    self.rdb.delete_file_info(feed)
                continue
                ...
                try:
                    ...
                    if '-' in ports_range:
                        ...
                        for port in range(first_port, last_port+1):
                            ...
                            self.rdb.set_organization_of_port(
    ...
    def __check_if_update_local_file(self, file_path: str) -> bool:
        ...
        file_info = self.rdb.get_TI_file_info(file_path)
    ...
    def __check_if_update_online_whitelist(self) -> bool:
        ...
        ti_file_info = self.rdb.get_TI_file_info('tranco_whitelist')
    ...
    def __check_if_update(self, file_to_download: str, update_period) -> bool:
        ...
        # Get the last time this file was updated
        ti_file_info = self.rdb.get_TI_file_info(file_to_download)
        last_update = ti_file_info.get('time', float('-inf'))
        ...
        try:
            ...
            if not new_e_tag:
                ...
                if new_last_modified != cached_last_modified:
                    ...
                    self.rdb.set_last_update_time(file_to_download, time.time())
                    self.loaded_ti_files += 1
                    return False
    ...
    def parse_ssl_feed(self, url, full_path):
        ...
        self.rdb.add_ssl_sha1_to_IoC(malicious_ssl_certs)
        return True
    ...
    async def update_TI_file(self, link_to_download: str) -> bool:
        ...
        try:
            ...
            self.rdb.set_TI_file_info(link_to_download, file_info)
            ...
            try:
                ...
                for indicator in response:
                    ...
                    if indicator.get('type', '') == 'domain':
                        ...
                        self.rdb.add_domains_to_IoC(malicious_domains_dict)
            except KeyError:
                self.print(
    ...
    def __delete_old_source_IPs(self, file):
        ...
        """
        all_data = self.rdb.get_IPs_in_IoC()
        old_data = []
    ...
    def __delete_old_source_Domains(self, file):
        ...
        if old_data:
            self.rdb.delete_domains_from_IoC_domains(old_data)
    ...
    def parse_ja3_feed(self, url, ja3_feed_path: str) -> bool:
        ...
        try:
            ...
            self.rdb.add_ja3_to_IoC(malicious_ja3_dict)
            return True
    ...
    def parse_json_ti_feed(self, link_to_download, ti_file_path: str) -> bool:
        ...
        if 'rstcloud' in link_to_download:
            ...
            self.rdb.add_ips_to_IoC(malicious_ips_dict)
            return True
    ...
    def parse_ti_feed(
        ...
        try:
            ...
            self.rdb.add_ips_to_IoC(malicious_ips_dict)
            self.rdb.add_domains_to_IoC(malicious_domains_dict)
            self.rdb.add_ip_range_to_IoC(malicious_ip_ranges)
            return True
    ...
    def check_if_update_org(self, file):
        cached_hash = self.rdb.get_TI_file_info(file).get('hash','')
    ...
    def get_whitelisted_orgs(self) -> list:
        ...
        whitelisted_orgs: dict = self.rdb.get_whitelist('organizations')
    ...
    def update_org_files(self):
        ...
        for org in utils.supported_orgs:
            ...
            for file in (org_ips, org_domains, org_asn):
                ...
                self.rdb.set_TI_file_info(file, info)
    ...
    def update_online_whitelist(self):
        ...
        with open(online_whitelist_download_path, 'r') as f:
            while line := f.readline():
                domain = line.split(',')[1]
                self.rdb.store_tranco_whitelisted_domain(domain)
    ...
    async def update(self) -> bool:
        ...
        try:
            ...
            self.rdb.set_loaded_ti_files(self.loaded_ti_files)
