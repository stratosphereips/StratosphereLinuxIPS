...
class ARP(IAsyncModule):
    ...
    async def init(self):
        ...
        self.is_zeek_running: bool = await self.is_running_zeek()
    ...
    async def new_arp_msg_handler(self, msg):
        ...
        if self.check_if_gratutitous_arp(flow):
            ...
            await self.detect_mitm_arp_attack(twid, flow)
        else:
            # not gratuitous and request, may be an arp scan
            await self.check_arp_scan(profileid, twid, flow)

        if "request" in flow.operation:
            await self.check_dstip_outside_localnet(twid, flow)
        elif "reply" in flow.operation:
            # Unsolicited ARPs should be of type reply only, not request
            await self.detect_unsolicited_arp(twid, flow)
    ...
    async def is_running_zeek(self) -> bool:
        return (
            await self.db.get_input_type() == "pcap"
            or await self.db.is_running_non_stop()
        )
    ...
    async def check_arp_scan(self, profileid, twid, flow):
        """
        ...
        if await self.db.get_gateway_ip() == flow.saddr:
            return False

        if flow.saddr == INADDR_ANY:
            return False
        ...
        if len(daddrs) >= self.arp_scan_threshold:
            ...
            if self.diff <= 30.00:
                ...
                if not self.alerted_once_arp_scan:
                    print("@@@@@@@@@@@@@@@@ alerting first time")
                    self.alerted_once_arp_scan = True
                    await self.set_evidence.arp_scan(
                        flow.starttime, profileid, twid, uids
                    )
                    # after we set evidence, clear the dict so we can detect
                    # if it does another scan
                    with self.cache_arp_lock:
                        try:
    ...
    async def check_dstip_outside_localnet(self, twid, flow):
        """Function to setEvidence when daddr is outside the local network"""

        if INADDR_ANY in flow.saddr or INADDR_ANY in flow.daddr:
        ...
        if not flow.daddr.startswith(first_octet):
            await self.set_evidence.dstip_outside_localnet(flow, twid)
            return True

        return False

    async def detect_unsolicited_arp(self, twid: str, flow):
        """
        ...
        if (
            ...
            await self.set_evidence.unsolicited_arp(flow, twid)
            return True

    async def detect_mitm_arp_attack(self, twid: str, flow):
        """
        ...
        original_ip: str = await self.db.get_ip_of_mac(flow.smac)
        ...
        if flow.saddr != original_ip:
            await self.set_evidence.mitm_arp_attack(flow, twid, original_ip)
            return True
    ...
    async def clear_arp_logfile(self):
        ...
        if (
            ...
            arp_log_file_path = os.path.join(
                await self.db.get_output_dir(), "zeek_files/arp.log"
            )
