...
class InMemoryTrustDatabase(TrustDatabase):
    ...
    def __init__(self, configuration: TrustModelConfiguration):
        ...
        self.__network_opinions: Dict[
            Target, Tuple[Time, SlipsThreatIntelligence]
        ] = {}
    ...
    def get_peers_with_organisations(
        ...
        return [
            p.info
            for p in self.__trust_matrix.values()
            if len(required.intersection(p.organisations)) > 0
        ]
    ...
    def get_peers_with_geq_recommendation_trust(
        ...
        return [
            p.info
            for p in self.__trust_matrix.values()
            if p.recommendation_trust >= minimal_recommendation_trust
        ]
    ...
    def get_peers_info(self, peer_ids: List[PeerId]) -> List[PeerInfo]:
        return [
            tr.info for p in peer_ids if (tr := self.__trust_matrix.get(p))
        ]
    ...
    def get_peers_with_geq_service_trust(
        ...
        return [
            p.info
            for p in self.__trust_matrix.values()
            if p.service_trust >= minimal_service_trust
        ]
    ...
    def get_cached_network_opinion(
        ...
        """Returns cached network opinion. Checks cache time and returns
        None if data expired."""
