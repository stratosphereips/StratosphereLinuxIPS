...
class IoCHandler:
    ...
    Contains all the logic related to setting and retrieving IoCs in the db
    """
    ...
    async def _build_trie(self):
        ...
        self.trie = Trie()
        ioc_domains: Dict[str, str] = await self.rcache.hgetall(
    ...
    async def set_loaded_ti_files(self, number_of_loaded_files: int):
        """
        Stores the number of successfully loaded TI files
        """
        await self.r.set(
            self.constants.LOADED_TI_FILES, number_of_loaded_files
        )

    async def get_loaded_ti_feeds_number(self):
        """
        Returns the number of successfully loaded TI files or 0 if none is loaded
        """
        return await self.r.get(self.constants.LOADED_TI_FILES) or 0

    async def delete_feed_entries(self, url: str):
        """
        Delete all entries in IoC_domains and IoC_ips that contain the given
        feed as source
        """
        # Get the feed name from the given url
        feed_to_delete = url.split("/")[-1]
        # Get all domains that are read from TI files in our db
        ioc_domains = await self.rcache.hgetall(self.constants.IOC_DOMAINS)
        for domain, domain_description in ioc_domains.items():
            ...
            if feed_to_delete in domain_description["source"]:
                ...
                await self.rcache.hdel(self.constants.IOC_DOMAINS, domain)
                self._invalidate_trie_cache()

        # Get all IPs that are read from TI files in our db
        ioc_ips = await self.rcache.hgetall(self.constants.IOC_IPS)
        for ip, ip_description in ioc_ips.items():
            ip_description = json.loads(ip_description)
            if feed_to_delete in ip_description["source"]:
                # This entry has the given feed as source, delete it
                await self.rcache.hdel(self.constants.IOC_IPS, ip)

    async def delete_ti_feed(self, file):
        await self.rcache.hdel(self.constants.TI_FILES_INFO, file)

    async def get_loaded_ti_feeds(self):
        """
        Returns the successfully loaded/cached TI files
        """
        return await self.rcache.hgetall(self.constants.TI_FILES_INFO)

    async def set_feed_last_update_time(self, file: str, time: float):
        """
        Sets the 'time' of last update of the given file
        :param file: TI file
        """
        if file_info := await self.rcache.hget(
            self.constants.TI_FILES_INFO, file
        ):
    ...
    async def get_ti_feed_info(self, file):
        """
        Get TI file info
        :param file: a valid filename not a feed url
        """
        data = await self.rcache.hget(self.constants.TI_FILES_INFO, file)
        return json.loads(data) if data else {}

    async def give_threat_intelligence(
        self,
        profileid,
        twid,
        ip_state,
        ...
        await self.publish(self.constants.GIVE_TI, json.dumps(data_to_send))
        return data_to_send

    async def set_ti_feed_info(self, file, data):
        """
        ...
        await self.rcache.hset(self.constants.TI_FILES_INFO, file, data)

    async def store_known_fp_md5_hashes(self, fps: Dict[str, List[str]]):
        await self.rcache.hmset(self.constants.KNOWN_FPS, fps)

    async def is_known_fp_md5_hash(self, hash: str) -> Optional[str]:
        """Returns the description of the given hash if it is a FP, and
        False if the hash is not a FP"""
        return (await self.rcache.hmget(self.constants.KNOWN_FPS, hash))[0]

    async def delete_ips_from_ioc_ips(self, ips: List[str]):
        """
        Delete the given IPs from IoC
        """
        await self.rcache.hdel(self.constants.IOC_IPS, *ips)

    async def delete_domains_from_ioc_domains(self, domains: List[str]):
        """
        Delete old domains from IoC
        """
        await self.rcache.hdel(self.constants.IOC_DOMAINS, *domains)
        self._invalidate_trie_cache()

    async def add_ips_to_ioc(
        self, ips_and_description: Dict[str, str]
    ) -> None:
        """
        ...
        if ips_and_description:
            await self.rcache.hmset(
                self.constants.IOC_IPS, ips_and_description
            )

    async def add_domains_to_ioc(self, domains_and_description: dict) -> None:
        """
    ...
    async def add_ip_range_to_ioc(self, malicious_ip_ranges: dict) -> None:
        """
    ...
    async def add_asn_to_ioc(self, blacklisted_ASNs: dict):
        """
        ...
        """
        if blacklisted_ASNs:
            await self.rcache.hmset(self.constants.IOC_ASN, blacklisted_ASNs)

    async def add_ja3_to_ioc(self, ja3: dict) -> None:
        """
        Store the malicious JA3 IoCs in the db
        :param ja3:  {ja3: {'source':..,'tags':..,
                            'threat_level':... ,'description'}}
        """
        await self.rcache.hmset(self.constants.IOC_JA3, ja3)

    async def add_jarm_to_ioc(self, jarm: dict) -> None:
        """
        Store the malicious JARM IoCs in the db
        :param jarm:  {jarm: {'source':..,'tags':..,
                            'threat_level':... ,'description'}}
        """
        await self.rcache.hmset(self.constants.IOC_JARM, jarm)

    async def add_ssl_sha1_to_ioc(self, malicious_ssl_certs):
        """
        Store a group of SSL fingerprints in the db
        :param malicious_ssl_certs:  {sha1: {'source':..,'tags':..,
                                    'threat_level':... ,'description'}}
        """
        await self.rcache.hmset(self.constants.IOC_SSL, malicious_ssl_certs)

    async def is_blacklisted_asn(self, asn) -> bool:
        return await self.rcache.hget(self.constants.IOC_ASN, asn)

    async def is_blacklisted_jarm(self, jarm_hash: str):
        """
        Search for the given hash in the malicious hashes stored in the db
        """
        return await self.rcache.hget(self.constants.IOC_JARM, jarm_hash)

    async def is_blacklisted_ip(self, ip: str) -> Union[Dict[str, str], bool]:
        """
        ...
        """
        ip_info: str = await self.rcache.hget(self.constants.IOC_IPS, ip)
        return False if ip_info is None else json.loads(ip_info)

    async def is_blacklisted_ssl(self, sha1):
        info = (await self.rcache.hmget(self.constants.IOC_SSL, sha1))[0]
        return False if info is None else info

    async def _match_exact_domain(
        self, domain: str
    ) -> Optional[Dict[str, str]]:
        """Checks if the given domain is blacklisted.
        Checks only the exact given domain, no subdomains"""
        domain_description = await self.rcache.hget(
    ...
    async def _match_subdomain(self, domain: str) -> Optional[Dict[str, str]]:
        """
        Checks if we have any blacklisted domain that is a part of the
        given domain
        Uses a cached trie for optimization.
        """
        # The goal here is we don't retrieve that huge amount of domains
        # from the db on every domain lookup
        # so we retrieve once, put them in a trie (aka cache them in memory),
    ...
    async def is_blacklisted_domain(
        self, domain: str
        ...
        """
        if match := await self._match_exact_domain(domain):
            is_subdomain = False
            return match, is_subdomain

        if match := await self._match_subdomain(domain):
            is_subdomain = True
            return match, is_subdomain
        return False, False

    async def get_all_blacklisted_ip_ranges(self) -> dict:
        """
        Returns all the malicious IP ranges we have from different feeds
        Return format is {range: json.dumps{'source':..,'tags':..,
                                            'threat_level':... ,'description'}}
        """
        return await self.rcache.hgetall(self.constants.IOC_IP_RANGES)

    async def get_all_blacklisted_ips(self):
        """
        Get all IPs and their description from IoC_ips
        """
        return await self.rcache.hgetall(self.constants.IOC_IPS)

    async def get_all_blacklisted_domains(self):
        """
        Get all Domains and their description from IoC_domains
        """
        return await self.rcache.hgetall(self.constants.IOC_DOMAINS)

    async def get_all_blacklisted_ja3(self):
        """
        Get all JA3 and their description from IoC_JA3
        """
        return await self.rcache.hgetall(self.constants.IOC_JA3)

    async def is_profile_malicious(self, profileid: str) -> str:
        return (
            await self.r.hget(profileid, self.constants.LABELED_AS_MALICIOUS)
            if profileid
            else False
        )

    async def is_cached_url_by_vt(self, url):
        """
        Return information about this URL
        Returns a dictionary or False if there is no URL in the database
        We need to separate these three cases:
        1- URL is in the DB without data. Return empty dict.
        2- URL is in the DB with data. Return dict.
        3- URL is not in the DB. Return False
        This is used to cache URL info by the VirusTotal module only
        """
        data = await self.rcache.hget(self.constants.VT_CACHED_URL_INFO, url)
        data = json.loads(data) if data else False
        return data

    async def _store_new_url(self, url: str):
        """
        ...
        """
        data = await self.is_cached_url_by_vt(url)
        if data is False:
            # If there is no data about this URL
            # Set this URL for the first time in the virustotal_cached_url_info
            # It's VERY important that the data of the first time we see a URL
            # must be '{}', an empty dictionary! If not, the logic breaks.
            # We use the empty dictionary to find if a URL exists or not
            await self.rcache.hset(
                self.constants.VT_CACHED_URL_INFO, url, "{}"
            )

    async def get_domain_data(self, domain):
        """
    ...
    async def _set_new_domain(self, domain: str):
        ...
        2- Publishes in the channels that there is a new domain, and that we want
            data from the Threat Intelligence modules
        """
        data = await self.get_domain_data(domain)
        if data is False:
            # If there is no data about this domain
            # Set this domain for the first time in the DomainsInfo
            # It's VERY important that the data of the first time we see a domain
            # must be '{}', an empty dictionary! If not, the logic breaks.
            # We use the empty dictionary to find if a domain exists or not
            await self.rcache.hset(self.constants.DOMAINS_INFO, domain, "{}")

    async def set_info_for_domains(
        ...
        for key in iter(info_to_set):
            ...
            if not isinstance(data_to_store, list):
                # data_to_store and prev_info should both be lists, so we can
                # extend
                data_to_store = [data_to_store]

            # If there is data previously stored, check if we have this key
            # already
            try:
    ...
    async def cache_url_info_by_virustotal(self, url: str, urldata: dict):
        ...
        if data is False:
            ...
            await self._store_new_url(url)
            # Now get the data, which should be empty, but just in case
            data = await self.get_ip_info(url)
        ...
        if dict_has_keys:
            ...
            await self.rcache.hset(
                self.constants.VT_CACHED_URL_INFO, url, urldata
            )
