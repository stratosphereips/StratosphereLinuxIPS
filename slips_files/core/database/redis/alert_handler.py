...
class AlertHandler():
    ...
    def set_evidence_causing_alert(self, profileid, twid, alert_ID, evidence_IDs: list):
        ...
        self.rdb.r.hset(f'{profileid}{self.separator}{twid}', 'alerts', profileid_twid_alerts)
        ...
        profile_alerts = self.rdb.r.hget('alerts', profileid)
        ...
        if not profile_alerts:
            ...
            self.rdb.r.hset('alerts', profileid, alert)
            return
        ...
        self.rdb.r.hset('alerts', profileid, profile_alerts)
    ...
    def get_evidence_causing_alert(self, profileid, twid, alert_ID) -> list:
        ...
        """
        if alerts := self.rdb.r.hget(f'{profileid}{self.separator}{twid}', 'alerts'):
    ...
    def set_flow_causing_evidence(self, uids: list, evidence_ID):
        self.rdb.r.hset("flows_causing_evidence", evidence_ID, json.dumps(uids))

    def get_flows_causing_evidence(self, evidence_ID) -> list:
        uids = self.rdb.r.hget("flows_causing_evidence", evidence_ID)
    ...
    def setEvidence(
        ...
        self.rdb.r.hset(f'evidence{profileid}', twid, current_evidence)
        ...
        if should_publish:
            self.rdb.r.incr('number_of_evidence', 1)
    ...
    def set_evidence_for_profileid(self, evidence):
        ...
        self.rdb.r.sadd('Evidence', evidence)
    ...
    def deleteEvidence(self, profileid, twid, evidence_ID: str):
        ...
        self.rdb.r.hset(f'evidence{profileid}', twid, current_evidence_json)
    ...
    def cache_whitelisted_evidence_ID(self, evidence_ID:str):
        ...
        self.rdb.r.sadd('whitelisted_evidence', evidence_ID)
    ...
    def is_whitelisted_evidence(self, evidence_ID):
        ...
        """
        return self.rdb.r.sismember('whitelisted_evidence', evidence_ID)
    ...
    def get_profileid_twid_alerts(self, profileid, twid) -> dict:
        ...
        """
        alerts = self.rdb.r.hget(f'{profileid}{self.separator}{twid}', 'alerts')
        if not alerts:
            return {}
        alerts = json.loads(alerts)
        return alerts
    ...
    def getEvidenceForTW(self, profileid, twid):
        ...
        evidence = self.rdb.r.hget(profileid + self.separator + twid, 'Evidence')
        if evidence:
    ...
    def update_threat_level(self, profileid, threat_level: str, confidence):
        ...
        self.rdb.r.hset(profileid, 'threat_level', threat_level)
        now = time.time()
        now = utils.convert_format(now, utils.alerts_format)
        # keep track of old threat levels
        confidence = f'confidence: {confidence}'
        past_threat_levels = self.rdb.r.hget(profileid, 'past_threat_levels')
        ...
        self.rdb.r.hset(profileid, 'past_threat_levels', past_threat_levels)
        ...
        if cached_ip_data := self.getIPData(ip):
            ...
            self.rdb.rcache.hset('IPsInfo', ip, json.dumps(cached_ip_data))
        else:
            self.rdb.rcache.hset('IPsInfo', ip, json.dumps(score_confidence))
