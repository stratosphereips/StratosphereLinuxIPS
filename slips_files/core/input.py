...
class Input(ICore):
    ...
    async def init(
        ...
        # in IAsyncModule
        self.channels = {
            "remove_old_files": self.remove_old_files_msg_handler,
    ...
    async def read_nfdump_output(self) -> int:
        """
        ...
        if not self.nfdump_output:
            ...
            await self.db.set_input_metadata({"total_flows": self.total_flows})
    ...
    async def get_earliest_line(self):
        """
    ...
    async def read_zeek_folder(self):
        ...
        self.bro_timeout = 10
        growing_zeek_dir: bool = await self.db.is_growing_zeek_dir()
        if growing_zeek_dir:
        ...
        for file in os.listdir(self.given_path):
            ...
            await self.db.add_zeek_file(full_path)
    ...
    async def handle_binetflow(self):
    ...
    async def handle_suricata(self):
    ...
    async def handle_zeek_log_file(self):
        """
        ...
        await self.db.add_zeek_file(self.given_path)
    ...
    def remove_old_files_msg_handler(self, msg):
        """
        This function recvs msgs from filemonitor.py to tell it that zeek
        changed the log files, it deletes old zeek-date.log files and
        clears slips' open handles and sleeps again
        """
        if not self.is_running_non_stop:
            # we only rotate interface log files
            return
        ...
        # for example the old log file should be  ./zeek_files/dns.2022-05
        # -11-14-43-20.log
        ...
        # ignored files have no open handle, so we should only delete them
        # from disk
    ...
    async def new_module_flow_handler(self, msg):
        """
        Read flows sent by any external module (for example the CYST module)
        Supported flows are of type zeek conn log
        is called automatically on new msgs to the 'new_module_flow' channel
        """
    ...
    async def main(self):
        ...
        it runs in a loop.
        """
        if self.input_type == "CYST":
            # cyst flows are handled by the new_module_flow_handler().
            return
        ...
        try:
            ...
            handler = input_handlers[self.input_type]
