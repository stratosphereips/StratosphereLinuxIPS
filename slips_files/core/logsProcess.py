...
class LogsProcess(multiprocessing.Process):
    ...
    def createProfileFolder(self, profileid):
        ...
        if not os.path.exists(profilefolder):
            ...
            self.addDataToFile(
                profilefolder + '/' + 'ProfileData.txt', 'Profiled IP: ' + ip
            )
            ...
            if ip_info:
                printable_ip_info = ', '.join(
                    '{} {}'.format(k, v) for k, v in ip_info.items()
                )
                self.addDataToFile(
                    profilefolder + '/' + 'ProfileData.txt',
                    'Info: ',
                    file_mode='a+',
                )
                ...
                if dstips:
                    ...
                    for key in data:
                        ...
                        self.addDataToFile(
                            profilefolder + '/' + twlog,
                            '\t{} ({} times). Info: {}'.format(
                                key, data[key], printable_ip_info
                            ),
                            file_mode='a+',
                            data_type='text',
                        )
