...
class InputProcess(multiprocessing.Process):
    ...
    def read_zeek_files(self) -> int:
        try:
            ...
            while True:
                ...
                # Go to all the files generated by Zeek and read 1 line from each of them
        ...
        try:
            ...
            for file in os.listdir(self.given_path):
                ...
                if not __database__.is_growing_zeek_dir():
                    ...
                    # subtract comment lines in zeek tab files, they shouldn't be considered flows
                    total_flows -= 9
    ...
    def handle_zeek_log_file(self):
        try:
            if not self.given_path.endswith(".log") or self.is_ignored_file(self.given_path):
                # unsupported file
                return False
            ...
            # no need for a bro timeout when given a log file, bro is not even running
            self.bro_timeout = float("inf")
