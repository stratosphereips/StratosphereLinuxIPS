...
class ProfilerProcess(multiprocessing.Process):
    ...
    def process_zeek_tabs_input(self, new_line: str) -> None:
        ...
        if 'conn.log' in new_line['type']:
            ...
            # is_gw_dst value is either T or F
            try:
                self.column_values['is_gw_dst'] = line[15]
            except KeyError:
                # if the zeek dhcp.log wasn't generated by slips, it wont contain the is_gw_dst flag
                pass
            ...
            self.column_values.update(
                {
                    ...
                    'is_gw_dst': line.get('is_gw_dst', ''),     # either true or false
                }
            )
