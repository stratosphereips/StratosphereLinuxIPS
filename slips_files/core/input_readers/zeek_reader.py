...
class ZeekReader(IInputReader):
    ...
    def init(
        ...
        self.start_rotator()
        # list to keep track of the started worker threads reading zeek
        # logs to close them on shutdown
        self.worker_threads = []
    ...
    def shutdown_gracefully(self):
        self.observer.stop()
        try:
            for _thread in self.zeek_threads + self.worker_threads:
                _thread.join(3)
        except Exception:
            pass
    ...
    def cache_nxt_line_in_file(self, filename: str, interface: str):
        """
        reads 1 line of the given file and stores in queue
        for sending to the profiler
    ...
    def read_zeek_files(self) -> int:
        lines = 0
        try:
            # will contain tuples with (timestamp, filename, line)
            # the goal of this is to always have the line with the earliest
            # ts when doing a pq.get()
            zeek_files_sorted_by_ts_pq = queue.PriorityQueue()
            # workers will draw from this q
            zeek_files_q = queue.Queue()

            self.zeek_files: Dict[str, str] = self.db.get_all_zeek_files()
            # zeek files that we're going to distribute among workers
            for filename, interface in self.zeek_files.items():
                zeek_files_q.put((filename, interface))
            ...
            # Worker function to read lines from files
            def worker():
                while not self.input_proc.should_stop():
                    try:
                        filename, interface = zeek_files_q.get(timeout=0.5)
                    except queue.Empty:
                        continue

                    if utils.is_ignored_zeek_log_file(filename):
                        zeek_files_q.task_done()
                        continue

                    # reads 1 line from the given file and cache it
                    # from in self.cache_lines
                    cached_line = self.cache_nxt_line_in_file(
                        filename, interface
                    )
                    # todo
                    if cached_line is not None:
                        line, ts = cached_line
                        zeek_files_sorted_by_ts_pq.put((ts, filename, line))

                    # requeue file for next read
                    zeek_files_q.put((filename, interface))
                    zeek_files_q.task_done()

            number_of_parallel_zeek_workers = 5
            for _ in range(number_of_parallel_zeek_workers):
                worker_thread = threading.Thread(target=worker, daemon=True)
                utils.start_thread(worker_thread, self.db)
            ...
            while not self.input_proc.should_stop():
                ...
                try:
                    ...
                    ts, file_with_earliest_flow, earliest_line = (
                        zeek_files_sorted_by_ts_pq.get(timeout=0.1)
                    )
                except queue.Empty:
                    continue
                ...
                self.cache_lines.pop(file_with_earliest_flow, None)
                self.file_time.pop(file_with_earliest_flow, None)

                # Get the new list of files. Since new files may have been
                # created by Zeek while we were processing them.
                self.zeek_files: Dict[str, str] = self.db.get_all_zeek_files()
                # ensure new files are added to work queue
                for filename, interface in self.zeek_files.items():
                    if (filename, interface) not in zeek_files_q.queue:
                        zeek_files_q.put((filename, interface))
